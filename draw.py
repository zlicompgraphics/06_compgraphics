...
def draw_polygons(polygons, screen, color):
    ...
    while point < len(polygons) - 2:
        surface_normal = calculate_normal(polygons, point)
        dproduct = dot_product(surface_normal, view_vector)
        if dproduct > 0:
            ...
            draw_line(int(polygons[point + 1][0]), int(polygons[point + 1][1]),
                      ...
                      screen, color)
            draw_line(int(polygons[point + 2][0]), int(polygons[point + 2][1]),
                      int(polygons[point][0]), int(polygons[point][1]),
                      screen, color)
        point += 3
...
def add_sphere(polygons, cx, cy, cz, r, steps):
    ...
    for lat in range(lat_start, lat_stop):
        for longt in range(longt_start, longt_stop + 1):
            ...
            if longt == 0 or longt == longt_stop + 1:  # poles
                if lat >= lat_stop / 2:
                    index = -int((lat - lat_stop / 2) * steps - longt)
                    add_polygon(polygons, points[index][0], points[index][1], points[index][2],
                                points[index - 1][0], points[index - 1][1], points[index - 1][2],
                                points[index - steps - 1][0], points[index - steps - 1][1],
